...
class i8080 {
    ...
    */
    ...
    PCHL() {
        ...
        this._clock += 5n;
        return { Disassemble: `PCHL`, Ticks: 5, InstructionMeta: { jumpPerformed: true, size: 1 } };
    }
    ...
    JUMP(expr, addr, mnemonic) {
        if (expr) {
            ...
            this._clock += 10n;
            return {
                Disassemble: `${mnemonic} 0x${addr.toString(16).padStart(4,'0')}`,
                Ticks: 10,
                InstructionMeta: { jumpPerformed: true, size: 3 },
            };
        }
        this._clock += 3n;
    ...
    CALL(expr, addr, mnemonic) {
        if (expr) {
            ...
            return {
                Disassemble: `${mnemonic} 0x${addr.toString(16).padStart(4,'0')}`,
                Ticks: 17,
                InstructionMeta: { callPerformed: true },
            };
        }
        this._clock += 11n;
    ...
    RET() {
        ...
        this._clock += 10n;
        return { Disassemble: `RET`, Ticks: 10, InstructionMeta: { returnPerformed: true } };
    }
    ...
    RETURN(expr, mnemonic) {
        if (expr) {
            this._programCounter = this._popWordFromStack();
            this._clock += 11n;
            return { Disassemble: `${mnemonic}`, Ticks: 11, InstructionMeta: { returnPerformed: true } };
        }
        this._clock += 5n;
    ...
    RST(vector) {
        ...
        this._clock += 11n;
        return {
            Disassemble: `RST 0x${vector.toString(16).padStart(4,'0')}`,
            Ticks: 11,
            InstructionMeta: { callPerformed: true },
        };
    }
    ...
    IN(port) {
        ...
        this.clock += 10n;
        return { Disassemble: `IN #${port.toString(16).padStart(2,'0')}`, Ticks: 10 };
    }
    ...
    OUT(port) {
        ...
        this._clock += 10n;
        return { Disassemble: `OUT $#${port.toString(16).padStart(2,'0')}`, Ticks: 10 };
    }
     ...
     */
     ExecuteNextInstruction() {
        ...
        return {
            InstructionMeta: result.InstructionMeta || {},
